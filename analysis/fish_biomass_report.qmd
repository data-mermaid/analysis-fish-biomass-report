...
project_name <- "Madagascar NW 2020"
...
datatable(fish_tbl %>%
            select(site, management_rules, biomass_kgha_avg))
```
...
# Get the 1/3 and 2/3 quantiles to compare with thresholds
quantile(x = biomass, probs = c(0.33, 0.66))
...
fish_histogram <- plot_ly() %>%
  # Below yellow threshold (red)
  add_histogram(
    x = biomass_truncated[biomass_truncated < yellow_threshold],
  ...
  add_histogram(
    x = biomass_truncated[biomass_truncated > green_threshold],
    xbins = list(start = 0, size = 100),
...
if (use_site_grouping) {
  ...
  if (!grouping_useful) {
    ...
    site_order <- fish_tbl %>%
      ...
      summarise(site_biomass = biomass_kgha_avg, .groups = "drop") %>%
      mutate(community = factor(community, levels = community_levels)) %>%
      arrange(community, desc(site_biomass))

    site_levels <- site_order$site
  }
...
fish_trophic_tbl <- fish_tbl %>%
  select(community, site, latitude, longitude,
...
trophic_biomass_plot <- ggplot(
  ...
  geom_text(data = fish_trophic_tbl,
            aes(x = site, y = biomass_kgha_avg + (max_biomass * 0.01),
...
fish_family_tbl <- fish_tbl %>%
  select(
    community, site, latitude, longitude,
    biomass_kgha_avg,
  ...
  group_by(community, site, latitude, longitude,
...
family_biomass_plot <- ggplot(
  ...
  aes(x = site, y = biomass, fill = family)
) +
  geom_bar(position = "stack", stat = "identity", alpha = 0.9,
           color = "black", linewidth = 0.25) +
  geom_hline(yintercept = yellow_threshold, colour = "#f3a224",
  ...
  geom_text(
    aes(x = site, y = biomass_kgha_avg + (max_biomass_fam * 0.01),
