...
R/Quarto template that pulls coral reef fishbelt transect data from [MERMAID](https://datamermaid.org) and produces report-ready visualizations for fish biomass reporting. Designed to work with any MERMAID project — just change the project tag and thresholds.

## Overview

This repository provides a single, configurable Quarto document that generates fish biomass report card figures from MERMAID sample event data. It authenticates with the MERMAID API via the [`mermaidr`](https://github.com/data-mermaid/mermaidr) R package and filters projects by a user-specified tag.

All project-specific settings (tag, thresholds, site grouping) are controlled through a Configuration section at the top of the document, so the code does not need to be edited elsewhere to adapt it to a new project.
...
|---|---|---|
| `project_tag` | MERMAID project tag to filter by | `"WCS Solomon Islands"` |
...
- A MERMAID account with access to at least one project matching your tag

## Usage

1. Open the `.Rproj` file in RStudio.
2. Edit the Configuration section in `analysis/fish_biomass_report.qmd` to set your project tag and thresholds.
